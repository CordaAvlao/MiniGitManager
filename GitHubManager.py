...
class GitHubManager:
    ...
    def upload_selection(self):
        ...
        if os.path.isdir(path):
            messagebox.showinfo("Info", "Folder upload not supported in this version. Select files.")
            return
        ...
        def _up():
            try:
                ...
                data = {"message": f"Upload {fname}", "content": b64}
                if sha: data["sha"] = sha
                
                url = f"https://api.github.com/repos/{self.current_repo}/contents/{remote_full_path}"
